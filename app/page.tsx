...
export default async function Page() {
  ...
  const roadblockRes = await fetch(`${backend_uri}/api/roadblocks/network/prabidhi.org`, options)
  ...
  const [heroData, categoryBlocks] = await Promise.all([
    fetch(`${backend_uri}/api/posts/featured-news/prabidhi.org`, options)
      .then(res => res.ok ? res.json() : { post: [] })
      .then(data => data?.post || [])
      .catch(() => []),

    fetch(`${backend_uri}/api/posts/homepage/prabidhi.org`, options)
      .then(res => res.ok ? res.json() : { post: [] })
      .then(data => data?.post || [])
      .catch(() => [])
  ])
